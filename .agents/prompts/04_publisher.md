...
- Save the final output strictly to `generated/revised/` using the exact filename format: `yyyy-MM-dd-{title}.md` (Pipeline Item #5).
...
- **Output**: Revised `.md` file saved in `generated/revised/`.

## 4. 팀 통신 프로토콜 (Team Communication Protocol)
- Hand over the path of the saved revised file in `generated/revised/` to the `Publisher` agent.

## 5. 에러 핸들링 (Error Handling)
- If an image reference in the text does not match any physical file in `generated/`, log an error and skip renaming for that specific asset.
...
- Confirm that the output file path strictly follows `generated/revised/yyyy-MM-dd-{title}.md` and contains both valid frontmatter and merged body.