...
#### 댓글창이 안 나올 때
위의 모든 설정을 다 했는 데에도 불구하고 어느 특정 포스트에 댓글창이 안 보일 때가 있다.

['성장에 한계를 느끼는 순간, 스스로 돌파구를 찾는 개발자가 되려면'](../lecture/2021-12-21-돌파구를%20찾는%20개발자가%20되려면.md) 포스트가 그랬다.

원인은 간단했다. 포스트의 파일 제목이 원래 '2021-12-21-성장에 한계를 느끼는 순간, 스스로 돌파구를 찾는 개발자가 되려면'이었다. 그런데 이는 파일명의 길이 때문에 생긴 일이었다.

아무 포스트의 페이지에 들어가서 개발자 도구의 'Network'를 클릭하자. 그 뒤, 개발자 도구를 닫지 않고 새로고침을 하면 이 페이지를 불러오는 데 쓰인 리소스들이 전부 보인다.  
그 중에 ```isseus?q=...```로 시작하는 파일이 있다. 더블 클릭해서 새 창으로 크게 보면
```
"total_count": 1,
...
```
이것과 같은 형식의 raw 파일이 나온다. 아마 json 형식 같다. 여기서 ```"title"``` 부분을 보면 어딘가 익숙한 문자열이 보이는데, 바로 이 포스트의 url이다.  
그런데, 이 포스트의 url이 한글임에도 불구하고 '%'에 숫자 영어가 조합되어 있다. 이건 한글을 바이트로 변환한 것으로, 중요한 건 **절대 title의 길이가 101자를 넘어서는 안 된다.**